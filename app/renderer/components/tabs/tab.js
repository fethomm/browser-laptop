...
function tabTransitions (duration, easing, zIndexDelay, ...props) {
  return props
    .map(prop => `${prop} ${duration} ${easing} 0s`)
    .join(', ') +
    `, z-index 0s linear ${zIndexDelay}`
}
...
const styles = StyleSheet.create({
  ...
  tabArea_isPreview: {
    ...
    transition: tabTransitions(
      ...
      // z-index should change immediate
      '0s',
      'box-shadow', 'transform', 'border', 'margin'
    )
  },
  ...
  tabArea__tab__identity: {
    ...
    transition: `margin ${theme.tab.transitionDurationOut} ${theme.tab.transitionEasingOut}`
  },
